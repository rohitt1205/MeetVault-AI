...
class MeetingCatalogService:
    ...
    @staticmethod
    def clear_catalog() -> dict:
        count = len(MeetingCatalogService._catalog)
        MeetingCatalogService._catalog = []
        MeetingCatalogService._onedrive_items_by_event_id = {}
        MeetingCatalogService._last_synced_at = None
        MeetingCatalogService._last_diagnostics = {}
        MeetingCatalogService._sync_status = "IDLE"
        MeetingCatalogService._sync_error = None
        return {"cleared_catalog_items": count}
    ...
    def _build_catalog(
        *,
        ...
        for file_item in onedrive_catalog_files:
            event_id = file_item.get("_catalog_event_id") or OneDriveService.catalog_event_id(file_item)
            ...
            catalog_by_id[event_id] = {
                ...
                "meeting_id": event_id,
                "title": (
                    file_item.get("_catalog_meeting_title")
                    or OneDriveService.display_title_from_filename(file_item.get("name") or "")
                ),
                "organizer": (
                    "Shared with me"
        ...
        MeetingCatalogService._catalog = visible_catalog
        ...
        MeetingCatalogService._last_diagnostics = {
            ...
            "recorded_catalog_candidates": len(catalog),
            "catalog_count": len(visible_catalog),
            "sync_error": sync_error,
